...
#Add new columns
data_7 <- Overseas %>% group_by(Match_Number, Striker) %>% 
  mutate(ball_faced = 1 ,
...
#d1 <- data_7 %>% filter(Striker == "TM Dilshan")
#view(d1)
...
data_8 <- data_7 %>% mutate(Striker_status =
                              case_when(Striker %in% batsman_vec ~ "batsman",
                                        Striker %in% bowler_vec ~ "bowler",
                                        Striker %in% allR_vec ~ "all rounder")
)

write.csv(data_8 , "~/Research/Research---ST426/Data/NewData/Merge/Overseas/Overseas.csv" , row.names = FALSE)
